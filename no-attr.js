var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        }
    }
    return to.concat(ar || Array.prototype.slice.call(from));
};
import { parse } from 'node-html-parser';
/**
 * 需要添加 base 路径的属性列表
 */
var NEED_BASE_PATH_ATTRS = [
    'src',
    'data-src',
    'href',
    'data-href',
    'content', // 用于 meta 标签
    'background',
    'poster',
    'cite',
    'action',
    'formaction',
];
/**
 * 需要移除的属性列表（这些属性在静态部署中会导致问题）
 */
var REMOVE_ATTRS = [
    'nomodule',
    'crossorigin',
    'integrity',
];
/**
 * 需要移除的标签选择器
 */
var REMOVE_TAGS = [
    'link[rel="modulepreload"]',
    'link[rel="prefetch"]',
    'link[rel="preload"]',
];
/**
 * 检查是否为外部链接
 */
function isExternalLink(url) {
    if (!url)
        return false;
    return url.startsWith('http://') ||
        url.startsWith('https://') ||
        url.startsWith('//') ||
        url.startsWith('data:') ||
        url.startsWith('blob:');
}
/**
 * 检查是否为需要添加 base 路径的资源
 */
function shouldAddBasePath(url, base, skipExternal) {
    if (!url || !base)
        return false;
    if (skipExternal && isExternalLink(url))
        return false;
    if (url.startsWith(base))
        return false;
    if (url.startsWith('/'))
        return false;
    return true;
}
/**
 * 处理标签属性
 */
function processAttributes(element, options) {
    var _a = options.base, base = _a === void 0 ? './' : _a, _b = options.enableBasePath, enableBasePath = _b === void 0 ? true : _b, _c = options.customBasePathAttrs, customBasePathAttrs = _c === void 0 ? [] : _c, _d = options.skipExternalLinks, skipExternalLinks = _d === void 0 ? true : _d;
    var basePathAttrs = __spreadArray(__spreadArray([], NEED_BASE_PATH_ATTRS, true), customBasePathAttrs, true);
    var removeAttrs = __spreadArray([], REMOVE_ATTRS, true);
    // 处理需要移除的属性
    removeAttrs.forEach(function (attrName) {
        if (element.hasAttribute(attrName)) {
            element.removeAttribute(attrName);
        }
    });
    // 特殊处理：将 type="module" 的 script 转换为普通脚本
    if (element.tagName === 'SCRIPT' && element.hasAttribute('type')) {
        var typeValue = element.getAttribute('type');
        if (typeValue === 'module') {
            // 移除 type 属性，使其成为普通脚本
            element.removeAttribute('type');
        }
    }
    // 处理需要添加 base 路径的属性
    if (enableBasePath && base && base !== '/') {
        basePathAttrs.forEach(function (attrName) {
            if (element.hasAttribute(attrName)) {
                var value = element.getAttribute(attrName);
                if (shouldAddBasePath(value, base, skipExternalLinks)) {
                    // 确保 base 路径以 / 结尾
                    var normalizedBase = base.endsWith('/') ? base : "".concat(base, "/");
                    var newValue = normalizedBase + value;
                    element.setAttribute(attrName, newValue);
                }
            }
        });
    }
}
/**
 * 处理 HTML 内容
 */
function processHtml(html, options) {
    var root = parse(html);
    var _a = options.customRemoveTags, customRemoveTags = _a === void 0 ? [] : _a;
    var removeTags = __spreadArray(__spreadArray([], REMOVE_TAGS, true), customRemoveTags, true);
    // 移除不需要的标签
    removeTags.forEach(function (selector) {
        var elements = root.querySelectorAll(selector);
        elements.forEach(function (element) {
            element.remove();
        });
    });
    // 处理 script 标签
    var scripts = root.querySelectorAll('script');
    scripts.forEach(function (script) {
        processAttributes(script, options);
    });
    // 处理 link 标签
    var links = root.querySelectorAll('link');
    links.forEach(function (link) {
        processAttributes(link, options);
    });
    // 处理 img 标签
    var images = root.querySelectorAll('img');
    images.forEach(function (img) {
        processAttributes(img, options);
    });
    // 处理 meta 标签（某些 meta 标签可能需要 base 路径）
    var metas = root.querySelectorAll('meta');
    metas.forEach(function (meta) {
        // 只处理 content 属性中的 URL
        var property = meta.getAttribute('property');
        var name = meta.getAttribute('name');
        if (property || name) {
            processAttributes(meta, options);
        }
    });
    // 处理 video 标签
    var videos = root.querySelectorAll('video');
    videos.forEach(function (video) {
        processAttributes(video, options);
        // 处理 video 内部的 source 标签
        var sources = video.querySelectorAll('source');
        sources.forEach(function (source) {
            processAttributes(source, options);
        });
    });
    // 处理 audio 标签
    var audios = root.querySelectorAll('audio');
    audios.forEach(function (audio) {
        processAttributes(audio, options);
        // 处理 audio 内部的 source 标签
        var sources = audio.querySelectorAll('source');
        sources.forEach(function (source) {
            processAttributes(source, options);
        });
    });
    // 处理 iframe 标签
    var iframes = root.querySelectorAll('iframe');
    iframes.forEach(function (iframe) {
        processAttributes(iframe, options);
    });
    // 处理 embed 标签
    var embeds = root.querySelectorAll('embed');
    embeds.forEach(function (embed) {
        processAttributes(embed, options);
    });
    // 处理 object 标签
    var objects = root.querySelectorAll('object');
    objects.forEach(function (object) {
        processAttributes(object, options);
    });
    // 处理 a 标签的 href 属性（可选，通常不需要）
    var anchors = root.querySelectorAll('a');
    anchors.forEach(function (anchor) {
        processAttributes(anchor, options);
    });
    return root.innerHTML;
}
/**
 * 创建 HTML 后处理插件
 */
export function htmlPostBuildPlugin(options) {
    if (options === void 0) { options = {}; }
    return {
        name: 'html-post-build',
        enforce: 'post',
        apply: 'build',
        transformIndexHtml: function (html) {
            return processHtml(html, options);
        },
    };
}
/**
 * 默认导出，使用默认配置
 */
export default htmlPostBuildPlugin;
